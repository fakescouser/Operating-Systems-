...
int processors[MAX_PROCESSORS] = {0};  
...
void fcfs() {
    ...
    while ((proc = proc_rq_dequeue()) != NULL) {
        ...
        if (i == MAX_PROCESSORS) {          
            printf("All processors are busy. Waiting...\n");
            sleep(1); 
        }
    }
}
...
/*void rr_aff() {
    ...
    while ((proc = proc_rq_dequeue()) != NULL) {
        if (proc->assigned_processor == -1) {
            ...
            int assigned = proc->assigned_processor;
            if (processors[assigned] == 0) {
                processors[assigned] = 1;  
                proc->t_start = proc_gettime();
                pid = fork();
                if (pid == -1) {
                    err_exit("fork failed!");
                ...
                if (pid == 0) {
                    ...
                    printf("executing %s on assigned processor %d\n", proc->name, assigned);
                    execl(proc->name, proc->name, NULL);
                } else {
                    proc->pid = pid;
                    proc->status = PROC_RUNNING;
